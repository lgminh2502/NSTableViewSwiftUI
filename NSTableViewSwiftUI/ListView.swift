...
final class NotListViewModel<Element: ListIdentifiable>: ObservableObject {
...
struct NotList<Element: ListIdentifiable>: NSViewRepresentable {
...
struct ListItem: ListIdentifiable {
    let id: UUID = UUID()
    let title: String
    let action: () -> Void
}
